  ...
  return (
    <nav className="min-w-[300px] min-h-screen bg-slate-50 border border-slate-200 shadow-sm font-Montserrat">
      ...
      <ul className="text-xs font-semibold pt-5">
        ...
        <li className="w-full px-3 mt-1">
         <button className={`py-3 px-3 rounded-xl w-full text-left uppercase hover:bg-slate-300 ${contentView === 'appointments' ? 'activeMenu'  : ''}`} onClick={() => setContentView('patients')}>Consultas</button>
        </li>
      </ul>
    </nav>
  );
